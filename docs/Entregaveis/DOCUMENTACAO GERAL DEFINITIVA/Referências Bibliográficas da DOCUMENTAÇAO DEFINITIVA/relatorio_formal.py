# -*- coding: utf-8 -*-
"""
RELATÓRIO ETL COMPLETO - PI-BETS - VERSÃO FORMAL
...
# ============================================================================
# CONFIGURAÇÕES INICIAIS - FORMAL
# ============================================================================

PDF_PATH = r"c:\Users\caioz\Downloads\ETL PIBETS\RELATORIO_ETL_PI_BETS_2026_FORMAL.pdf"
COLORS = {
    'primary': HexColor('#000000'),      # Preto (formal)
    'secondary': HexColor('#333333'),    # Cinza escuro (formal)
    'roxo_tabelas': HexColor('#a7197f'),  # Roxo (apenas em tabelas)
    'light_gray': HexColor('#f3f4f6'),
    'dark_gray': HexColor('#374151'),
    'text': HexColor('#000000'),         # Preto
}
...
# Estilos Customizados - TOTALMENTE FORMAL
...
def create_header():
    """Cria capa do documento com logo CEUB"""
    elements = []
    ...
    # Logo CEUB (via URL)
    try:
        logo_url = "https://www.ceub.br/sites/default/files/logo-ceub.png"
        logo_path = "ceub_logo.png"
        if not os.path.exists(logo_path):
            urllib.request.urlretrieve(logo_url, logo_path)
        if os.path.exists(logo_path):
            img = Image(logo_path, width=2.5*cm, height=1.5*cm)
            img.hAlign = 'CENTER'
            elements.append(img)
            elements.append(Spacer(1, 0.3*cm))
    except:
        pass  # Continuar sem logo se não conseguir baixar
    
    # Título principal
    elements.append(Paragraph(
        "RELATÓRIO TÉCNICO: ANÁLISE ETL COMPLETA E DETALHADA",
        title_style
    ))
    elements.append(Spacer(1, 0.3*cm))
    ...
    info_data = [
        ["Instituição:", "Centro de Ensino Unificado de Brasília - UniCEUB"],
        ["Curso:", "Ciência da Computação - 5º Semestre"],
        ["Disciplina:", "Projeto Integrador I"],
        ["Data de Entrega:", datetime.now().strftime("%d de %B de %Y")],
        ["Versão:", "2.0 - Formal"],
    ]
    ...
    # Sem rodapé de coração - apenas informação técnica
    elements.append(Paragraph(
        "Relatório Formal - Projeto de Pesquisa Integrada",
        ParagraphStyle(
            'Footer',
            parent=styles['Normal'],
            fontSize=9,
...
def create_index():
    """Cria índice"""
    elements = []
    elements.append(Paragraph("Índice de Conteúdos", heading1_style))
    elements.append(Spacer(1, 0.3*cm))
    ...
    for item in index_items:
        elements.append(Paragraph(item, body_left))
...
def create_visao_geral():
    ...
    elements = []
    elements.append(Paragraph("1. Visão Geral Executiva", heading1_style))
    ...
    <br/><br/>
    - Integrar dados de múltiplas fontes (Banco Central, DataSUS, Consumidor.gov.br, Serasa)
    ...
    impacto_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), COLORS['roxo_tabelas']),
...
def create_contexto():
    ...
    elements.append(Paragraph("2. Contexto Histórico e Institucional", heading1_style))
...
def create_linha_tempo():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("3. Linha do Tempo: Evolução do Projeto", heading1_style))
...
def create_arquitetura_etl():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("4. Arquitetura ETL (Extract-Transform-Load)", heading1_style))
    ...
    <br/>- Decodificação: Conversão de ZIP em CSVs individuais e importação
    <br/>- Normalização de Encoding: UTF-8 e Latin1 para garantir compatibilidade
    <br/>- Filtragem Semântica: Busca por keywords ('BET', 'APOSTA', 'CASSINO', 'BLAZE')
    <br/>- Conversão de Tipos: Numéricos (valores monetários), datas, booleanos
    <br/>- Tratamento de Missings: NaN e valores inválidos removidos ou imputados
    <br/>- Agregação Temporal: Agrupamento por mês/ano para séries históricas
    <br/>- Simulação Estatística: Geração de dados sintéticos quando real indisponível
    """
    ...
    <br/>- Caches em Memória: Streamlit cache_data() para performance
    <br/>- DataFrames Pandas: Estrutura tabular para análise
    <br/>- Visualizações Matplotlib/Seaborn: Gráficos interativos
    <br/>- Tabelas Dinâmicas: Filtros por gênero e faixa etária
    <br/>- Download de Dados: Exportação em CSV para pós-análise
    """
...
def create_codigo_detalhado():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("5. Análise Detalhada de Código", heading1_style))
    ...
    elements.append(Paragraph(
        "Streamlit: Framework web para aplicações Python (interativas e rápidas)<br/>"
        "Pandas: Manipulação de dados tabulares (CSVs, DataFrames)<br/>"
        "NumPy: Computações numéricas e simulações estatísticas<br/>"
        "Matplotlib/Seaborn: Visualização de gráficos estáticos e interativos<br/>"
        "ZipFile/Glob: Processamento em lote de arquivos",
        body_style
    ))
    elements.append(Spacer(1, 0.3*cm))
    ...
    <br/>- Variáveis de cor (bg_body, text_main, border_color)
    <br/>- Parâmetros matplotlib customizados para gráficos visíveis no escuro
    <br/>- Cores do Modo Escuro: Fundo #0f172a (azul-escuro), Superfícies #1e293b (cinza), Texto #f8fafc (branco)
    <br/>- CSS Injected: Força aplicação de cores usando !important em elementos Streamlit
    """
    ...
    <br/>Tenta carregar dados_sus.csv. Se não encontrado, gera dados sintéticos com distribuição normal (idade) e lognormal (renda)
    <br/><br/>
    
    <b>Função 2: get_dados_consumidor_local()</b>
    <br/>Extrai arquivo ZIP bases_consumidor.zip, concatena múltiplos CSVs em um único DataFrame, 
    filtra registros com keywords ('BET', 'APOSTA', 'CASSINO', 'BLAZE'), conta frequência de problemas relatados (saque, publicidade, bloqueio, etc.)
    <br/><br/>
    
    <b>Função 3: get_dados_consolidados_csv()</b>
    <br/>Lê dados_apostas_consolidado.csv, converte coluna 'Valor' para numérico (erro coercion ignorado). 
...
def create_transformacoes():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("6. Transformações de Dados (Detalhamento)", heading1_style))
    ...
    <br/>Código: zip_ref.extractall(pasta_extracao)
    <br/><br/>
    ...
    <br/>Resultado: Reduz 1M de registros para ~50k reclamações sobre BETs
    <br/><br/>
    
    <b>5. AGREGAÇÃO (GROUP BY)</b>
    <br/>Transformação: Contagem de problemas por categoria
    <br/>Código: df_bets['Problema'].value_counts().head(5)
    ...
    <br/>Código: pd.to_numeric(df['Valor'], errors='coerce')
    <br/>Converte com tolerância a erros (NaN se inválido)
    <br/><br/>
    
    <b>7. IMPUTAÇÃO ESTATÍSTICA</b>
    <br/>Quando dados reais indisponíveis, gera dados sintéticos com distribuição normal para idade (média 28, desvio 8) 
    e distribuição lognormal para renda (simula cauda longa realista). Dívida é calculada como Renda × 2 (hipótese de comprometimento).
    """
...
def create_kpis():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("7. KPIs e Indicadores", heading1_style))
    
    kpi_data = [
        ["KPI", "Métrica", "Valor", "Fonte", "Interpretação"],
        ["Endividados", "% apostadores com dívida", "86%", "Serasa", "Comprometimento financeiro crítico"],
        ["Ideação Suicida", "% em tratamento com ideação", "80%", "PRO-AMJO", "Risco de saúde mental extremo"],
        ["Volume Mercado", "Total movimentado anualmente", "R$ 120 Bi", "Cálculo indireto", "Magnitude econômica"],
        ["Perfil Jovem", "% apostadores (18-39 anos)", "56%", "Pesquisa", "Vulnerabilidade de adultos"],
        ["Reclamações", "Saques bloqueados", "1.450", "Consumidor.gov", "Abuso de plataforma"],
        ["Poupança", "% que desvia savings", "52%", "PwC 2024", "Comprometimento de emergência"],
        ["Essencial", "% reduz lazer/alimentação", "48%", "PwC 2024", "Privação severa"],
        ["Crescimento", "Aumento ideação 2018-2025", "+53 p.p.", "Comparativo", "Tendência alarmante"],
    ]
    
    kpi_table = Table(kpi_data, colWidths=[1.5*cm, 2*cm, 1.5*cm, 1.8*cm, 4.2*cm])
...
def create_mapas_mentais():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("8. Mapas Mentais do Trajeto", heading1_style))
    ...
    <br/><b>Financeiras:</b> Endividamento familiar (86%), Corte de essenciais (48%), Depleção de poupança (52%)
    <br/><b>Psicológicas:</b> Ludopatia (F63.0 CID-10), Ideação suicida (80%), Depressão associada
    <br/><b>Sociais:</b> Fragilização familiar, Ciclo de pobreza, Saúde pública comprometida
    <br/><br/>
    ...
    elements.append(Paragraph(mapa1_text, body_style))
    elements.append(Spacer(1, 0.3*cm))
    ...
    elements.append(Paragraph(mapa2_text, body_style))
...
def create_descobertas():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("9. Descobertas Principais", heading1_style))
...
def create_como_usar():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("10. Como as Pessoas Utilizarão a Solução", heading1_style))
    
    usuarios = [
        {
            "nome": "Legisladores e Policy-Makers",
            "como": [
...
def create_conclusoes():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("11. Conclusões e Próximos Passos", heading1_style))
    ...
    <br/>
    "O vício em apostas, potencializado pela tecnologia (IHC) e facilidade PIX, 
    contribui significativamente para o comprometimento financeiro das famílias brasileiras."
    """
    ...
    <br/>- Mobile App: Alertas para usuários sobre gasto em apostas
    <br/>- Chatbot: Triagem de ludopatia com referência a tratamento
    <br/>- Chrome Extension: Aviso ao tentar acessar plataformas
    <br/>- Dashboard de Monitoramento: Para autoridades federais (tempo real)
    """
...
def create_referencias():
    ...
    elements = []
    elements.append(PageBreak())
    elements.append(Paragraph("12. Referências Bibliográficas", heading1_style))
    
    referencias = [
        ["Organização", "Ano", "Título", "Fonte"],
...
def main():
    print("Gerando Relatório ETL Formal em PDF...")
    ...
    print(f"Relatório formal gerado com sucesso!")
    print(f"Caminho: {PDF_PATH}")
    print(f"Total de seções: 12")
    print(f"Formatação: Completamente formal (sem cores rosas, sem emojis, sem bullets decorativos)")
    print(f"Tabelas: Com cor roxa (apenas tabelas)")
