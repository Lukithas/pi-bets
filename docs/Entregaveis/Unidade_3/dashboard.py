...
def get_data_estatistica():
    ...
    # Lê o CSV que você gerou no Colab
    ...
    # Fallback seguro: se não encontrar o CSV, gera os dados simulados
    np.random.seed(42)
...
if sus_encontrado:
    ...
    st.sidebar.warning("⚠️ SUS (CSV) não encontrado. A usar Mockup.")
...
# Filtros aplicados após carregar os dados
...
with tab1:
    ...
    if not df_pacientes.empty:
        ...
        media_idade = f"{int(df_pacientes['Idade'].mean())} anos"
        render_dashboard_card(col3, "3. Perfil Etário de Risco", "Média de Idade", media_idade, fig3, "<strong>📌 Análise:</strong> Predominância de adultos jovens, com concentração entre 20 e 35 anos.")

        fig4, ax4 = plt.subplots(figsize=(6, 3))
        sns.boxplot(data=df_pacientes, x='Genero', y='Divida_Acumulada', ax=ax4, palette="muted")
        render_dashboard_card(col4, "4. Endividamento por Género", "Impacto", "Variável", fig4, "<strong>📌 Análise:</strong> Homens apresentam maior dispersão de dívida.")
    ...
    if not df_pacientes.empty and 'Renda_Mensal' in df_pacientes.columns:
        ...
        render_dashboard_card(col5, "5. Renda vs Dívida", "Correlação", "Direta", fig5, "<strong>📌 Análise:</strong> O endividamento cresce proporcionalmente à renda.")
...
with tab2:
    ...
    st.subheader("1. Microdados (DataSUS - F63.0)")
    ...
    st.download_button("📥 Baixar Pesquisa da Equipa (CSV)", data=convert_df_to_csv(df_consolidado), file_name='dados_apostas_consolidado.csv', mime='text/csv')
